...
public class BudgetMapperImplementation implements BudgetMapper
{
	...
	{
		return Budget.builder ().userId (budgetModel.getUserId ()).category (budgetModel.getCategory ()).limitAmount (budgetModel.getLimitAmount ()).build ();
	}
	@Override
	public BudgetDTO toBudgetDTO (Budget budget)
	{
		return BudgetDTO.builder ().id (budget.getId ()).userId (budget.getUserId ()).category (budget.getCategory ()).limitAmount (budget.getLimitAmount ()).build ();
	}
}